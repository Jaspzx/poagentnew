...
class SiteAuditReport extends RevisionableContentEntityBase implements SiteAuditReportInterface {
  ...
  static public function audit($format = 'html') {
    $reportDefinitions = \Drupal::service('plugin.manager.site_audit_report')->getDefinitions();
    ...
    if (!empty($saved_reports) &&
      ...
      foreach ($saved_reports as $saved_report) {
        if ($saved_report) {
          $reports[$saved_report] = \Drupal::service('plugin.manager.site_audit_report')->createInstance($saved_report);
        }
      }
    }
    // There are no reports selected, so run them all.
      ...
      foreach ($reportDefinitions as $reportDefinition) {
        $reports[$reportDefinition['id']] = \Drupal::service('plugin.manager.site_audit_report')->createInstance($reportDefinition['id']);
      }
    }
    ...
    switch ($format) {
      case 'html':
        $renderer = new Html($reports, $logger, $options);
        $render = $renderer->render(TRUE);
        $out = \Drupal::service('renderer')->renderRoot($render);
        break;

      case 'json';
        foreach ($reports as $plugin_id => $report) {
          $renderer = new Json($report, $logger, $options, $output);
      ...
      case 'markdown':
        foreach ($reports as $report) {
          $renderer = new Markdown($report, $logger, $options, $output);
      ...
      default:
        foreach ($reports as $report) {
          $renderer = new Console($report, $logger, $options, $output);
